...
export class ChariotComponent implements OnInit {
  ...
  ngOnInit(): void {
    [this.uniqListArticle, this.count] = this.getUniqueArray(
      this.shoppingList.getSelectedBooks()
    );
    this.calculPrix();
    this.getSimilairBooks();
  }

  supprimerLivre(b: Book): void {
  ...
  calculPrix(): void {
    [this.PrixTotal, this.Remise] = this.promotionService.calculOffre(
      this.shoppingList.getSelectedBooks()
    );
  }

  // afficher notification
  showToaster(titre: string): void {
    this.toastr.warning('Le livre ' + titre + ' a été supprimé du panier');
  }

  getSimilairBooks(): void {
    this.bookService.getBooks().subscribe((data) => {
      this.similairBooks = data.filter(
        (a) => !this.uniqListArticle.map((b) => b.title).includes(a.title)
      );
    });
  }
