...
enum TypesOfOffres {"slice", "percentage", "minus"}
...
export class PromotionService {
  ...
  calculOffre( selectedBooks : Book[]){
    ...
    this.promotion.forEach(offre=>{
      if(offre.type==TypesOfOffres.percentage && this.offreInitial - (this.offreInitial/100)*offre.value>0){
        //calcul offre pourcentage
        offrepercentage=  this.offreInitial - (this.offreInitial/100)*offre.value
      }
      if(offre.type==TypesOfOffres.minus &&  this.offreInitial - offre.value > 0){
        //calcul offre minus
        offreminus=  this.offreInitial - offre.value
      }
      if(offre.type==TypesOfOffres.slice &&  this.offreInitial - Math.trunc(this.offreInitial/offre.sliceValue)*offre.value> 0){
        //calcul offre slice
        offreslide=  this.offreInitial - Math.trunc(this.offreInitial/offre.sliceValue)*offre.value
      }
    })
